# Department CRUD - Spring Boot Application

A Spring Boot REST API to **Store / Update / Retrieve / Delete** department records
(`id`, `name`) backed by **MySQL**, documented with **Swagger (springdoc-openapi)**,
...
└── src
    ├── main
    │   ├── java/com/example/departmentcrud
    │   │   ├── DepartmentCrudApplication.java
    ...
    │   │   └── service/ (DepartmentService, impl/DepartmentServiceImpl)
    │   └── resources/application.properties
    └── test
        ├── java/com/example/departmentcrud
        │   ├── DepartmentCrudApplicationTests.java     # full integration test (H2)
        │   ├── controller/DepartmentControllerTest.java # @WebMvcTest
        │   └── service/DepartmentServiceImplTest.java   # Mockito unit test
...
### IntelliJ IDEA
1. `File → Open` → select the `department-crud` folder (the one containing `pom.xml`).
...
6. Run `DepartmentCrudApplication.java` (right-click → Run), or use the Maven side panel.

### Eclipse / Spring Tool Suite (STS)
1. `File → Import → Maven → Existing Maven Projects` → select `department-crud`.
...
4. Right-click `DepartmentCrudApplication.java → Run As → Spring Boot App`.

### VS Code
1. Install extensions: *Extension Pack for Java*, *Spring Boot Extension Pack*, *Lombok Annotations Support*.
2. Open the `department-crud` folder.
3. Use the Spring Boot Dashboard or `Run → Start Debugging` on `DepartmentCrudApplication`.
...
1. Install MySQL 8 locally, or run it via Docker:
   ```bash
   docker run --name mysql-department -e MYSQL_ROOT_PASSWORD=root -p 3306:3306 -d mysql:8.0
   ```
2. The app is configured (see `src/main/resources/application.properties`) to:
   - connect to `jdbc:mysql://localhost:3306/departmentdb`
   - auto-create the database (`createDatabaseIfNotExist=true`)
   - auto-create/update the `department` table (`spring.jpa.hibernate.ddl-auto=update`)
   ...
   ```properties
   spring.datasource.url=jdbc:mysql://localhost:3306/departmentdb?createDatabaseIfNotExist=true&useSSL=false&serverTimezone=UTC
...
|-----------------------------------|----------------------------|--------|
| `DepartmentServiceImplTest`          | Unit (Mockito)             | create, get-by-id (found/not-found), get-all, update (found/not-found), delete (found/not-found) |
| `DepartmentControllerTest`           | Web slice (`@WebMvcTest` + MockMvc) | all 5 endpoints, success + validation (400) + not-found (404) paths |
| `DepartmentCrudApplicationTests`     | Integration (`@SpringBootTest` + H2) | Spring context loads; full create → read → update → delete lifecycle through real HTTP calls |
...
```bash
mvn -Dtest=DepartmentServiceImplTest test
```
...
From inside the `department-crud` folder:

```bash
git init
git add .
git commit -m "Initial commit: Department CRUD Spring Boot app with Swagger and unit tests"
...
|--------|-------------------------------|---------------------------|-------------|
| POST   | `/api/v1/departments`             | Create a department           | `{"name":"Human Resources"}` |
| GET    | `/api/v1/departments/{id}`        | Retrieve one department       | – |
| GET    | `/api/v1/departments`              | Retrieve all departments      | – |
| PUT    | `/api/v1/departments/{id}`        | Update a department            | `{"name":"Human Resources & Talent"}` |
...
### Error handling demo
- `GET /api/v1/departments/999` (non-existent id) → `404 Not Found` with a JSON error body
  (`timestamp`, `status`, `error`, `message`, `path`).
- `POST /api/v1/departments` with `{"name":""}` → `400 Bad Request` with
...
- Add pagination to `GET /api/v1/departments` if the dataset can grow large.
